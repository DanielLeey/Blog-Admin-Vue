      ...
      <el-tab-pane v-permission="'/systemConfig/getSystemConfig'" name="one">
        ...
        <el-form style="margin-left: 20px;" label-position="left" label-width="140px" >
          ...
          <el-form-item label="详情图片显示优先级">
            <el-radio v-for="item in detailPicturePriorityDictList" :key="item.uid" v-model="form.contentPicturePriority" :label="item.dictValue" border size="medium">{{ item.dictLabel }}</el-radio>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      yesNoDictList: [], // 是否字典
      openDictList: [], // 开启关闭字典
      picturePriorityDictList: [], // 封面图片显示优先级
      detailPicturePriorityDictList: [], // 详情图片显示优先级
      startEmailNotification: [], // 网站消息邮件通知
      openDashboardNotification: [], // 仪表盘弹框通知
      openEmailActivate: [], // 注册用户邮件激活
  ...
  methods: {
    ...
    getDictList: function() {
      var dictTypeList = ['sys_search_model', 'sys_yes_no', 'sys_picture_priority', 'sys_de_pic_priority', 'sys_storage_region', 'sys_normal_disable', 'sys_editor_modal', 'sys_email_notify', 'sys_dash_notify', 'sys_email_activate']
      ...
      getListByDictTypeList(dictTypeList).then(response => {
        if (response.code === 200) {
          ...
          this.detailPicturePriorityDictList = dictMap.sys_de_pic_priority.list
          this.editorModalDictList = dictMap.sys_editor_modal.list
          this.searchModelDictList = dictMap.sys_search_model.list
          this.startEmailNotification = dictMap.sys_email_notify.list
          this.openDashboardNotification = dictMap.sys_dash_notify.list
          this.openEmailActivate = dictMap.sys_email_activate
        }
      })
    },
